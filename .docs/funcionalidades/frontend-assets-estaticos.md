...
Além de ícones e `logo_quadrado.png`, o repositório inclui variantes de marca (`TaktChat*.png`, `taktchat-logo_150x150.png`), `robots.txt`, `sitemap.xml`, a pasta `originais/` (cópias de referência) e `landing/` (prints públicos da vitrine: hero e galeria). Os prints do player `/apresentacoes` **não** ficam em `public/` — estão em `backend/private/kit-apresentacoes/` e exigem login. Evite nomes com espaços em novos ficheiros na raiz (`logo quadrado.png` existe por histórico local — preferir `logo_quadrado.png`).
