...
const FlowBuilderSingleBlockModal = ({
  ...
  const handleChangeMediasImg = (e, number) => {
    ...
    // Se era uma imagem que já existia, tira o id de elementsSeqEdit
    ...
    // document.querySelector(`.btnImg${number}`).remove(); // Linha removida conforme solicitado
  };
  ...
  const handleChangeAudios = (e, number) => {
    ...
    // document.querySelector(`.btnAudio${number}`).remove(); // Linha removida conforme solicitado
  };
  ...
  const handleChangeVideos = (e, number) => {
    ...
    divConteudo.innerHTML = `<video controls style="width: 200px;">
  ...
  // --- SUBSTITUA TODA a função antiga por esta -----------------
  const verifyButtonsUpload = () => {
    // Verifica se existe algum card de mídia onde:
    // 1) O botão de upload ainda está presente
    // 2) E NÃO há preview carregado (src/innerHTML vazios)
    const newArrImg = elementsSeq.filter(item => item.includes("img"));
    const newArrAudio = elementsSeq.filter(item => item.includes("audio"));
    const newArrVideo = elementsSeq.filter(item => item.includes("video"));
    ...
    // Imagens
    for (let i = 0; i < numberImg; i++) {
      const id = newArrImg[i];
      const imgTag = document.querySelector(`.img${id.replace('img', '')}`);
      const btnTag = document.querySelector(`.btn${capitalize(id)}`);
      if (btnTag && imgTag && !imgTag.src.includes('blob') && !imgTag.src.includes('/public/')) {
        return true; // card de imagem vazio
      }
    }
    // Áudios
    for (let i = 0; i < numberAudio; i++) {
      const id = newArrAudio[i];
      const audioDiv = document.querySelector(`.audio${id.replace('audio', '')}`);
      const btnTag = document.querySelector(`.btn${capitalize(id)}`);
      if (btnTag && audioDiv && audioDiv.innerHTML.trim() === "") {
        return true; // card de audio vazio
      }
    }
    // Vídeos
    for (let i = 0; i < numberVideo; i++) {
      const id = newArrVideo[i];
      const videoDiv = document.querySelector(`.video${id.replace('video', '')}`);
      const btnTag = document.querySelector(`.btn${capitalize(id)}`);
